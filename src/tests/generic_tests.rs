...
use crate::basic::cls::generic_type::{GenericTypeParameter, GenericType, GenericTypeContext, TypeBound, Variance};
...
fn test_type_constraint_validation() {
    ...
    let param = GenericTypeParameter::with_bounds("T".to_string(), vec![TypeBound::Class(int_type.clone())]);
...
fn test_generic_type_creation() {
    ...
    let type_args = vec![
        Class::Basic(BasicType::Number(NumberType::SignedInteger(SignedIntegerType::Int))),
...
fn test_generic_type_instantiation() {
    ...
    match instantiated {
        Class::Generic(instantiated_generic) => {
            ...
            assert_eq!(instantiated_generic.get_type_arguments(), type_args.as_slice());
        }
